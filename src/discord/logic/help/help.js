...
async function cmds(interaction) {
    ...
    if (interaction.isCommand()) await interaction.reply({ embeds: [embed], components: [helpButtons] });
    else if (interaction.isButton()) await interaction.update({ embeds: [embed], components: [helpButtons] });
